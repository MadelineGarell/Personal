...
array2 = np.array([[a, b, c, d, e], [f, g, h, i, j], [k, l, m, n, o], [p, q, r, s, t], [u, v, w, x, y]])
array[2,:]
array[:,2]
np.size(array2)
array = array * u.m
...
def stokeseinstein(temperature, radius, viscosity):
  temperature = tempurature * u.kelvin
  radius = radius * u.m
...
xArray = u.Quantity(np.arrange(273, 473, 10), u.kelvin)

@u.wraps(None, [u.m **3/u.s, u.m**2/u.s], False)
def re_flat_plate(velocity, dist, nu):
  """This function calculates the Reynolds Number for flow past a plate"""
  return(veolcity*dist/nu)

plt.plot(xArray, re_flat_plate(2, 0.4, pc.viscosity_kinematic(xArray)),
plt.xlabel('Temperature [K]')
plt.ylabel('Reynolds Number [m^2/s]')
plt.title('Relationship between Temperature and Reynolds Number')
plt.minorticks_on()
plt.grid(which = 'major')
plt.grid(which = 'minor')
plt.legend.(loc = 'lower right', ncol = 1)
plt.tight_layout()
plt.savefig('./Images/Temperature_ReynoldsNumber_Plot.png')
plt.show()
